...
struct JsonDecoder<Element: Codable> {
    func loadData(from fileName: String, of fileType: String) -> Element? {
        ...
        guard let filePath = try? loadFilePath(from: fileName, of: fileType) else { return nil }
        guard let data = try? loadFileData(from: filePath) else { return nil }
        guard let jsonData = try? decodeFileData(from: data) else { return nil }
    ...
    private func loadFileData(from filePath: String) throws -> Data? {
        guard let data = try? Data(contentsOf: URL(fileURLWithPath: filePath)) else { throw JsonParsingError.dataLoadError }
        return data
    }
